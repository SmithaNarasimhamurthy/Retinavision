...
# Modify final layer
densenet121.classifier = nn.Linear(1024, 5)
    ...
    if len(images) > 0:  # Otherwise predict button is available and the possibility of forward pass without input data.
        ...
        if st.button("**Predict**", help="The model evaluates all the given images and returns predicted classes and respective confidence scores."):
            ...
            for cat, prob in zip(output[0], output[1]):
                disease_label = disease_classes[cat]  # Get disease name from class index
                confidence = round(float(prob[cat]) * 100, 2)
                predictions.append(disease_label)
            ...
            st.dataframe(df, use_container_width=True, column_order=['Filename', 'Predicted Disease', 'Confidence (%)'])
